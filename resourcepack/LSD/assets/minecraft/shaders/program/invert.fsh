...
vec3 noise3(float inputF) {
    return vec3(
        noise(vec3(inputF)),
        noise(vec3(inputF + 10.0)),
        noise(vec3(inputF + 20.0))
    );
}
...
vec4 julia() {
    vec2 c = vec2(cos(time * 0.015) * 0.124, sin(time * 0.015) * 0.124 - 1 );
    vec2 z = (texCoord - vec2(0.5)) * 2.0;
    int i;
    for (i = 0; i < 32 && length(z) < 2.0; i++) {
        z = vec2(
            z.x * z.x - z.y * z.y + c.x,
            -2 * z.x * z.y + c.y
        );
    }
    return texture2D(DiffuseSampler, texCoord + vec2(float(i) * 0.04));
}
